...
| 6 | Fixed | `services/obsidian/requirements.txt` / Dockerfile | Unpinned fastapi/uvicorn/httpx and unpinned `obsidian-headless` in image build. | Originally pinned via `requirements.txt`; later revision removed that duplicate manifest — Dockerfile now uses `uv export --frozen --package obsidian-vault` against the workspace `uv.lock`, and still pins `obsidian-headless@0.0.14`. Covered by `test_vault_service_pins_python_and_node_dependency_versions`. |
...
| 9 | Fixed | `files.py` | Path check uses resolve then IO uses unresolved join; Sync TOCTOU note. | Superseded by O_NOFOLLOW descriptor-relative IO in `paths.py` (`open_file_under_roots` / `list_dir_under_roots`); symlink leaf/intermediate tests added. |
